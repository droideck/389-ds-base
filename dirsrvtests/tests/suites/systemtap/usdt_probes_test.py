...
# --- END COPYRIGHT BLOCK ---
"""Static USDT probe checks via readelf and stap -p1."""
...
def _systemtap_targets_or_skip(topo):
...
def test_usdt_probes_compiled_in(topo):
    ...
    :id: 2731a5ad-4cec-43cc-84f4-11cba9987c8a
    :setup: Standalone instance with --enable-systemtap
    :steps:
...
def test_probe_argument_counts_match(topo):
    ...
    :id: 86042a3a-81e4-458d-ab4c-0aca72c712bd
    :setup: Standalone instance with --enable-systemtap
    :steps:
...
def test_no_unexpected_probes(topo):
    ...
    :id: 5c01f5e7-364f-479b-ae26-21e161ba3222
    :setup: Standalone instance with --enable-systemtap
    :steps:
...
def test_stp_scripts_parse(topo):
    ...
    :id: a14ccb2c-8b1d-4aba-8b19-c2de24c32fde
    :setup: Standalone instance with --enable-systemtap, stap installed
    :steps:
        1. Glob profiling/stap/*.stp
