...
def test_deferred_update_exclude_subtree_honored(topo):
    ...
    :steps:
        ...
        2. Create a group inside the excluded subtree and a user outside it
        3. Add the user as a member of the group
        4. Check the user's memberOf attribute
    :expectedresults:
        1. Success
        2. Success
        3. Success
        4. memberOf is not added, the group is excluded from the scope
    """
    inst = topo.standalone
    ...
    group = Groups(inst, EXCLUDED_SUBTREE, rdn=None).create(properties={
    ...
    time.sleep(2)
    values = {v.lower() for v in user.get_attr_vals_utf8('memberOf')}
    assert group.dn.lower() not in values, \
