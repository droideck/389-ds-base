...
def lookup_disabled(inst):
    """Disable the optional equality lookup when the server exposes it.

    A clean baseline has no such configuration attribute.  In that case the
    context deliberately becomes a no-op while the caller still repeats the
    identical functional search.
    """
