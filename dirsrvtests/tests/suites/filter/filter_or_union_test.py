...
"""
Result correctness for OR filters whose candidate union spans many member
ID lists.  The live-singleton case is a deterministic 100-branch scale model
of issue #6275's 1000-equality shape.  These pin the exact result sets of
idl_set_union's consumers: the general k-way path, the preserved 2-list fast
path, the paged-results ALLIDS replacement, and a union consumed by an
enclosing AND intersection.
"""
...
def test_or_many_live_singletons(topo, create_users):
    """Verify an OR of many existing uid equalities (many singleton ID
    lists, a scaled form of issue #6275's shape) returns exactly those entries
...
def test_or_many_absent(topo, create_users):
    """Verify an OR dominated by absent values (empty member lists) still
    returns exactly the entries of its live values
...
def test_or_two_lists_fastpath(topo, create_users):
    """Verify the 2-list union fast path (idl_union, the issue #5170 SSSD
    shape) returns the exact result set for an OR of two multi-ID
    equality lists
