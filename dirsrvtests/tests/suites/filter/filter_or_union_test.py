...
ID lists (issue #6275's shape is an OR of 1000 uid equalities - 1000
singleton lists). These pin the exact result sets of idl_set_union's
consumers: the general k-way path, the preserved 2-list fast path, the
paged-results ALLIDS replacement, and a union consumed by an enclosing
AND intersection.
"""
...
def test_or_many_live_singletons(topo, create_users):
    ...
    lists, issue #6275's literal shape) returns exactly those entries
