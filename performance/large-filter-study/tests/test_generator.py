...
class AdditiveWorkloadTests(unittest.TestCase):
    """The drift canary and sub-threshold rungs never disturb legacy output."""

    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory(
            prefix="large-filter-additive-test-"
        )
        cls.root = Path(cls.temporary.name)
        spec = load_spec(DEFAULT_SPEC)
        legacy_spec = copy.deepcopy(spec)
        del legacy_spec["drift_canary"]
        del legacy_spec["matrices"]["sub_threshold_branch_counts"]
        legacy_dir = cls.root / "legacy-spec"
        legacy_dir.mkdir()
        (legacy_dir / "legacy-spec.json").write_text(
            json.dumps(legacy_spec, indent=2) + "\n"
        )
        workload_root = DEFAULT_SPEC.parent
        for name in ("schema", "indexes"):
            destination = legacy_dir / name
            destination.mkdir()
            for source in (workload_root / name).iterdir():
                (destination / source.name).write_bytes(source.read_bytes())
        cls.legacy_manifest = generate_workload(
            legacy_dir / "legacy-spec.json", "tiny", cls.root / "tiny-legacy",
        )
        cls.extended_manifest = generate_workload(
            DEFAULT_SPEC, "tiny", cls.root / "tiny-extended",
        )
    ...
    def test_legacy_artifacts_byte_identical_between_specs(self) -> None:
        legacy_files = self.legacy_manifest["files"]
        extended_files = self.extended_manifest["files"]
        changed = [
            name for name, digest in legacy_files.items()
            if extended_files.get(name) != digest
        ]
        # The copied spec necessarily differs; nothing else may.
        self.assertEqual(changed, ["study-spec.json"])
        self.assertEqual(
            sorted(set(self.legacy_manifest["scenarios"])
                   - set(self.extended_manifest["scenarios"])),
            [],
        )
        for name, legacy_entry in self.legacy_manifest["scenarios"].items():
            self.assertEqual(
                legacy_entry, self.extended_manifest["scenarios"][name], name
            )

    def test_canary_scenarios_cover_all_index_variants(self) -> None:
        variants = (
            "baseline-no-presence", "presence-sdn1", "presence-sdn2",
            "presence-both", "without-sdn1-equality",
            "without-sdn2-equality",
        )
        canary_ids = self.extended_manifest["scenario_groups"]["drift-canary"]
        self.assertEqual(
            sorted(canary_ids),
            sorted(f"drift-canary-{variant}" for variant in variants),
        )
        rendered = set()
        for variant in variants:
            scenario = self.extended_manifest["scenarios"][
                f"drift-canary-{variant}"
            ]
            self.assertEqual(scenario["index_variant"], variant)
            self.assertEqual(scenario["groups"], ["drift-canary"])
            self.assertEqual(scenario["expected_count"], 0)
            self.assertEqual(scenario["logical_outer_cohort_count"], 0)
            self.assertEqual(
                scenario["expected_lookup_diagnostic"]["expectation"],
            ...
            self.assertEqual(scenario["parameters"]["branch_count"], 8)
            self.assertTrue(scenario["parameters"]["drift_canary"])
            rendered.add(scenario["expected_sha256"])
        # Every variant executes the identical filter and result oracle.
        self.assertEqual(len(rendered), 1)

    def test_sub_threshold_rungs_follow_ladder_conventions(self) -> None:
        for count in (4, 8):
            for distribution in (
                ...
                self.assertIn("branch-scaling", scenario["groups"])
                self.assertIn(f"branch-{distribution}", scenario["groups"])
                self.assertEqual(
                    scenario["parameters"]["branch_count"], count
                )
                self.assertEqual(
                    scenario["expected_lookup_diagnostic"]["expectation"],
                    "forbidden",
                )
            zero = self.extended_manifest["scenarios"][
                f"branch-count-{count}-zero-candidate"
            ]
            self.assertIn("branch-zero-candidate", zero["groups"])
            self.assertEqual(zero["logical_outer_cohort_count"], 0)

    def test_spec_validation_rejects_threshold_violations(self) -> None:
        spec = load_spec(DEFAULT_SPEC)
        at_threshold = copy.deepcopy(spec)
        at_threshold["matrices"]["sub_threshold_branch_counts"] = [4, 16]
        with self.assertRaisesRegex(SpecificationError, "below the 16-branch"):
            validate_spec(at_threshold, "tiny", DEFAULT_SPEC)

        duplicated = copy.deepcopy(spec)
        duplicated["matrices"]["sub_threshold_branch_counts"] = [15]
        with self.assertRaisesRegex(SpecificationError, "duplicates"):
            validate_spec(duplicated, "tiny", DEFAULT_SPEC)

        canary = copy.deepcopy(spec)
        canary["drift_canary"]["branch_count"] = 16
        with self.assertRaisesRegex(SpecificationError, "below the 16-branch"):
            validate_spec(canary, "tiny", DEFAULT_SPEC)
