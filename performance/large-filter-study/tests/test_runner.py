...
class FullScreenWrapperContractTests(unittest.TestCase):
    """Text contracts for the additive full-screen wrapper changes."""

    @classmethod
    def setUpClass(cls) -> None:
        cls.wrapper = (
            STUDY_ROOT / "bin" / "run-full-fedora-screen"
        ).read_text(encoding="utf-8")

    def test_summarizer_heredoc_replaced_by_screen_report_module(self):
        self.assertIn('study/screen_report.py', self.wrapper)
        self.assertIn(
        ...
        # The old inline summarizer must not linger alongside the module.
        self.assertNotIn("screen_summary = {", self.wrapper)

    def test_provenance_preflight_runs_before_package_mutation(self):
        provenance = self.wrapper.index("study/provenance.py")
        first_restore = self.wrapper.index("sudo rpm --restore")
        self.assertLess(provenance, first_restore)
        self.assertIn("LF_EXPECTED_SOURCE", self.wrapper)
        self.assertIn("merge-base --is-ancestor", self.wrapper)
        self.assertIn("describe --always --dirty", self.wrapper)
        self.assertIn("provenance.json", self.wrapper)
        self.assertIn("provenance-preflight.log", self.wrapper)
        # Mismatch aborts; the unset case only warns.
        self.assertIn("PROVENANCE MISMATCH", self.wrapper)
        self.assertIn("WARNING: LF_EXPECTED_SOURCE is not set", self.wrapper)

    def test_canary_selection_is_probe_gated(self):
        self.assertIn("canary_prefix=drift-canary", self.wrapper)
        self.assertIn('scenario_groups', self.wrapper)
        self.assertIn(
            '--scenario "${canary_prefix}-${index_config}"', self.wrapper,
        )
        self.assertIn("regenerate the workload to enable", self.wrapper)

    def test_usage_documents_expected_source(self):
        self.assertIn("LF_EXPECTED_SOURCE expected custom source commit", self.wrapper)
    ...
    def test_wrapper_bash_syntax(self):
        ...
        completed = subprocess.run(
            [bash, "-n", str(STUDY_ROOT / "bin" / "run-full-fedora-screen")],
            capture_output=True, text=True,
        )
        self.assertEqual(completed.returncode, 0, completed.stderr)


class EnvironmentSidecarOrderTests(unittest.TestCase):
    """The environment capture stays outside every timed section."""

    @classmethod
    def setUpClass(cls) -> None:
        cls.source = (
            STUDY_ROOT / "study" / "run_study.py"
        ).read_text(encoding="utf-8")

    def test_start_snapshot_follows_incomplete_marker(self):
        incomplete = self.source.index('(output / "INCOMPLETE").write_text')
        start = self.source.index(
            "environment_start = capture_environment_snapshot()"
        )
        workload_copy = self.source.index(
            "copy_workload_payload(source_workload"
        )
        self.assertLess(incomplete, start)
        self.assertLess(start, workload_copy)

    def test_sidecar_written_after_artifacts_before_complete(self):
        correctness = self.source.index('write_json(output / "correctness.json"')
        sidecar = self.source.index("write_environment_sidecar(output,")
        complete = self.source.index('(output / "COMPLETE").write_text')
        self.assertLess(correctness, sidecar)
        self.assertLess(sidecar, complete)

    def test_capture_never_runs_inside_measure_one(self):
        measure_start = self.source.index("def measure_one(")
        following = self.source.index("\ndef ", measure_start + 1)
        measure_body = self.source[measure_start:following]
        self.assertNotIn("capture_environment_snapshot", measure_body)
