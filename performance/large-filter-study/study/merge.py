...
def _aa_created_at(run_manifest: Mapping[str, Any]) -> Optional[str]:
    value = run_manifest.get("created_at")
...
def _aa_session_sort_key(
        run_id: str, created_at: Optional[str]) -> tuple[int, str, str]:
    if created_at is None:
        return (1, "", run_id)
    return (0, created_at, run_id)


def _aa_state_signature(bundle: Bundle) -> str:
    server = bundle.run_manifest.get("server") or "unknown"
    lookup = (
        bundle.run_manifest.get("lookup_mode_actual")
        or bundle.run_manifest.get("lookup_mode_requested")
        or "unknown"
    )
    executable = ""
    server_executable = bundle.artifact_manifest.get("server_executable")
    if isinstance(server_executable, Mapping):
        executable = str(
            server_executable.get("executable_sha256") or ""
        )[:12]
    return f"{server}:{executable}:{lookup}"


def _aa_pass_order(
        pairs: Sequence[Mapping[str, Any]],
        bundles_by_run_id: Mapping[str, Bundle]) -> Dict[str, Any]:
    basis = "earliest created_at per state within each session index"
    unknown = {"status": "unknown", "state_orders_by_pass": [], "basis": basis}
    if not pairs:
        return unknown
    ...
    for session_key in ("session_a", "session_b"):
        ...
        for pair in pairs:
            session = pair[session_key]
            bundle = bundles_by_run_id.get(session["run_id"])
            created = session.get("created_at")
            if bundle is None or created is None:
                return unknown
            signature = _aa_state_signature(bundle)
            if signature not in state_times or created < state_times[signature]:
                state_times[signature] = created
        orders.append([
            signature for signature, _ in
            sorted(state_times.items(), key=lambda item: item[1])
        ])
    if len(orders[0]) == 1 and len(orders[1]) == 1:
        status = "single-state"
    elif orders[0] == orders[1]:
        status = "forward-repeat"
    elif orders[0] == list(reversed(orders[1])):
        status = "reverse"
    else:
        status = "mixed"
    return {"status": status, "state_orders_by_pass": orders, "basis": basis}


def _aa_session_summary(
        label: str, run_id: str, rows: Sequence[Mapping[str, Any]],
        bundle: Optional[Bundle]) -> Dict[str, Any]:
    elapsed = [row["metrics"]["elapsed_seconds"] for row in rows]
    cpu = [
        row["metrics"]["server_cpu_seconds"] for row in rows
        if row["metrics"]["server_cpu_seconds"] is not None
    ]
    created = _aa_created_at(bundle.run_manifest) if bundle else None
    harness = None
    schedule_design = None
    schedule_position = None
    if bundle is not None:
        identity = bundle.artifact_manifest.get("harness_identity")
        if isinstance(identity, Mapping):
            value = identity.get("content_sha256")
            harness = str(value) if value is not None else None
        schedule_design = bundle.run_manifest.get("schedule_design")
        schedule_position = bundle.run_manifest.get("schedule_position")
    return {
        "session_label": label,
        "run_id": run_id,
        "created_at": created,
        "source_directories": sorted({
            row["source_directory"] for row in rows
        }),
        "schedule_design": schedule_design,
        "schedule_position": schedule_position,
        "harness_content_sha256": harness,
        "n": len(elapsed),
        "elapsed_median_seconds": (
            statistics.median(elapsed) if elapsed else None
        ),
        "server_cpu_median_seconds": (
            statistics.median(cpu) if cpu else None
        ),
        "all_rows_release_eligible": all(
            row.get("release_eligible") for row in rows
        ),
    }


def _aa_distribution(values: Sequence[Optional[float]]) -> Dict[str, Any]:
    magnitudes = sorted(
    ...
    return {
        ...
        "p95_abs_fraction": nearest_rank(magnitudes) if magnitudes else None,
        "max_abs_fraction": magnitudes[-1] if magnitudes else None,
    }


def _aa_session_label(index: int) -> str:
    if index < 26:
        return chr(ord("A") + index)
    return f"S{index}"


def build_aa_comparisons(
        rows: Sequence[Mapping[str, Any]],
        bundles: Sequence[Bundle]) -> Dict[str, Any]:
    """Decompose already-pooled same-configuration strata into per-session
    medians and report their deltas.

    Diagnostic only: pooling (`_summary_group_key` ignores run_id), release
    eligibility, gates, and classifications are untouched.  Strata whose
    scenarios all belong to the ``drift-canary`` group are excluded from
    pairing — the canary repeats inside one session by design — and are
    reported separately as a repeatability sequence.
    """
    bundles_by_run_id = {bundle.run_id: bundle for bundle in bundles}
    grouped: Dict[str, list[Mapping[str, Any]]] = {}
    for row in rows:
        if row["phase"] != "measured":
            continue
        correctness = row.get("correctness") or {}
        if not correctness.get("passed") or not correctness.get("complete"):
            continue
        if row["metrics"]["elapsed_seconds"] is None:
            continue
        grouped.setdefault(_summary_group_key(row), []).append(row)

    def _run_key(run_id: str) -> tuple[int, str, str]:
        bundle = bundles_by_run_id.get(run_id)
        created = _aa_created_at(bundle.run_manifest) if bundle else None
        return _aa_session_sort_key(run_id, created)

    def _is_canary_stratum(members: Sequence[Mapping[str, Any]]) -> bool:
        scenario_groups: set[str] = set()
        for row in members:
            scenario_groups.update(row.get("scenario_groups") or [])
        return bool(scenario_groups) and scenario_groups <= {"drift-canary"}

    pairs: list[Dict[str, Any]] = []
    total_strata = 0
    multi_session = 0
    canary_strata: list[Dict[str, Any]] = []
    canary_deltas: list[Optional[float]] = []
    for key in sorted(grouped):
        members = grouped[key]
        total_strata += 1
        ...
        ordered_runs = sorted(by_run, key=_run_key)
        if _is_canary_stratum(members):
            sequence = []
            for run_id in ordered_runs:
                run_rows = by_run[run_id]
                elapsed = [
                    row["metrics"]["elapsed_seconds"] for row in run_rows
                ]
                bundle = bundles_by_run_id.get(run_id)
                sequence.append({
                    "run_id": run_id,
                    "created_at": (
                        _aa_created_at(bundle.run_manifest)
                        if bundle else None
                    ),
                    "schedule_position": (
                        bundle.run_manifest.get("schedule_position")
                        if bundle else None
                    ),
                    "n": len(elapsed),
                    "elapsed_median_seconds": (
            ...
            for first, second in zip(sequence, sequence[1:]):
                canary_deltas.append(_relative_change(
                    second["elapsed_median_seconds"],
                    first["elapsed_median_seconds"],
                ))
            canary_strata.append({
                "stratum_summary_id": key[:16],
                "scenario_ids": sorted({
                    row["scenario_id"] for row in members
                }),
        ...
        multi_session += 1
        labels = {
            run_id: _aa_session_label(index)
            for index, run_id in enumerate(ordered_runs)
        }
        for first, second in zip(ordered_runs, ordered_runs[1:]):
            session_a = _aa_session_summary(
                labels[first], first, by_run[first],
                bundles_by_run_id.get(first),
            )
            session_b = _aa_session_summary(
                labels[second], second, by_run[second],
                bundles_by_run_id.get(second),
            )
            harness_match = None
            if (
                session_a["harness_content_sha256"]
                and session_b["harness_content_sha256"]
            ):
                harness_match = (
                    session_a["harness_content_sha256"]
                    == session_b["harness_content_sha256"]
                )
            sample = by_run[first][0]
            pairs.append({
                "aa_pair_id": sha256_bytes(canonical_json_bytes({
                    "stratum": key, "run_a": first, "run_b": second,
                }))[:16],
                "stratum_summary_id": key[:16],
                "scenario_ids": sorted({
                    row["scenario_id"] for row in members
                }),
                "scenario_fingerprint": sample["scenario_fingerprint"],
                "configuration": sample["configuration"],
                "executable_sha256": sample["executable_sha256"],
                "behavioral_runtime_identity_sha256": sample[
                    "behavioral_runtime_identity_sha256"
                ],
                "host_signature": sample.get("host_signature"),
                "harness_identity_match": harness_match,
                "session_a": session_a,
                "session_b": session_b,
                "elapsed_median_delta_fraction": _relative_change(
                    session_b["elapsed_median_seconds"],
                    session_a["elapsed_median_seconds"],
                ),
                "server_cpu_median_delta_fraction": _relative_change(
                    session_b["server_cpu_median_seconds"],
                    session_a["server_cpu_median_seconds"],
                ),
            })
    return {
        "format_version": 1,
        "semantics": (
            "diagnostic-only same-configuration session deltas; never "
            "release evidence; never an acceptance-gate input"
        ),
        "release_evidence": False,
        "gate_input": False,
        "session_order_basis": (
            "run-manifest created_at ascending; missing timestamps fall "
            "back to run_id order"
        ),
        "total_stratum_count": total_strata,
        "multi_session_stratum_count": multi_session,
        "pair_count": len(pairs),
        "pass_order": _aa_pass_order(pairs, bundles_by_run_id),
        "pairs": pairs,
        "delta_distribution": {
            "elapsed_median": _aa_distribution([
                pair["elapsed_median_delta_fraction"] for pair in pairs
            ]),
            "server_cpu_median": _aa_distribution([
                pair["server_cpu_median_delta_fraction"] for pair in pairs
            ]),
        },
        "drift_canary": (
            {
                "stratum_count": len(canary_strata),
                "strata": canary_strata,
                "consecutive_delta_distribution": _aa_distribution(
                    canary_deltas
                ),
            }
            if canary_strata else None
        ),
    }
...
def _render_aa_section(aa: Optional[Mapping[str, Any]]) -> list[str]:
    lines = [
        "",
        "## A/A same-configuration deltas (diagnostic, non-release)",
        "",
        "Same-configuration strata measured by two or more separate sessions "
        "are decomposed into per-session medians. These deltas quantify "
        "run-to-run noise for one identical binary and configuration; they "
        "are never release evidence and never acceptance-gate inputs.",
        "",
    ]
    if not aa or not aa.get("pair_count"):
        lines.append(
            "No same-configuration multi-session pairs were detected in "
            "this merge."
        )
    else:
        elapsed_dist = aa["delta_distribution"]["elapsed_median"]
        cpu_dist = aa["delta_distribution"]["server_cpu_median"]
        pass_order = aa.get("pass_order") or {}
        lines.extend([
            f"- Multi-session strata: {aa['multi_session_stratum_count']} "
            f"of {aa['total_stratum_count']}; session pairs: "
            f"{aa['pair_count']}.",
            f"- Session order basis: {aa['session_order_basis']}.",
            "- Pass order across sessions: "
            f"{pass_order.get('status', 'unknown')}.",
            "- Elapsed median |delta|: median "
            f"{_fraction(elapsed_dist['median_abs_fraction'])}, p95 "
            f"{_fraction(elapsed_dist['p95_abs_fraction'])}, max "
            f"{_fraction(elapsed_dist['max_abs_fraction'])} "
            f"(n={elapsed_dist['n']}).",
            "- Server CPU median |delta|: median "
            f"{_fraction(cpu_dist['median_abs_fraction'])}, p95 "
            f"{_fraction(cpu_dist['p95_abs_fraction'])}, max "
            f"{_fraction(cpu_dist['max_abs_fraction'])} (n={cpu_dist['n']}).",
            "",
            "| scenario aliases | lookup | A run | A position | B run | "
            "B position | A n | B n | A median (s) | B median (s) | "
            "elapsed delta | CPU delta | notes |",
            "|---|---|---|---|---|---|---:|---:|---:|---:|---:|---:|---|",
        ])
        for pair in aa["pairs"]:
            session_a = pair["session_a"]
            session_b = pair["session_b"]
            notes = []
            if pair.get("harness_identity_match") is False:
                notes.append("harness differs")
            lines.append(
                "| {scenarios} | {lookup} | {a_run} | {a_pos} | {b_run} | "
                "{b_pos} | {a_n} | {b_n} | {a_median} | {b_median} | "
                "{elapsed_delta} | {cpu_delta} | {notes} |".format(
                    scenarios=_markdown(", ".join(pair["scenario_ids"])),
                    lookup=_markdown(
                        pair["configuration"].get("lookup_mode")
                    ),
                    a_run=_markdown(session_a["run_id"][:12]),
                    a_pos=_markdown(session_a.get("schedule_position")),
                    b_run=_markdown(session_b["run_id"][:12]),
                    b_pos=_markdown(session_b.get("schedule_position")),
                    a_n=session_a["n"],
                    b_n=session_b["n"],
                    a_median=_seconds(session_a["elapsed_median_seconds"]),
                    b_median=_seconds(session_b["elapsed_median_seconds"]),
                    elapsed_delta=_fraction(
                        pair["elapsed_median_delta_fraction"]
                    ),
                    cpu_delta=_fraction(
                        pair["server_cpu_median_delta_fraction"]
                    ),
                    notes=_markdown(", ".join(notes)) if notes else "—",
                )
            )
    canary = (aa or {}).get("drift_canary")
    lines.extend(["", "### Drift-canary repeatability", ""])
    if not canary:
        lines.append("Unavailable: no drift-canary strata in this merge.")
    else:
        dist = canary["consecutive_delta_distribution"]
        lines.append(
            f"- Canary strata: {canary['stratum_count']}; consecutive-run "
            "elapsed |delta|: median "
            f"{_fraction(dist['median_abs_fraction'])}, p95 "
            f"{_fraction(dist['p95_abs_fraction'])}, max "
            f"{_fraction(dist['max_abs_fraction'])} (n={dist['n']})."
        )
        for stratum in canary["strata"]:
            lines.extend([
                "",
                f"Stratum `{stratum['stratum_summary_id']}` "
                f"({_markdown(', '.join(stratum['scenario_ids']))}, lookup "
                f"{_markdown(stratum['configuration'].get('lookup_mode'))}):",
                "",
                "| run | position | created_at | n | median elapsed (s) |",
                "|---|---|---|---:|---:|",
            ])
            for run in stratum["runs"]:
                lines.append(
                    f"| {_markdown(run['run_id'][:12])} | "
                    f"{_markdown(run.get('schedule_position'))} | "
                    f"{_markdown(run.get('created_at'))} | {run['n']} | "
                    f"{_seconds(run['elapsed_median_seconds'])} |"
                )
    return lines
...
def render_results(
        ...
        minimum_repeats: int,
        aa_comparisons: Optional[Mapping[str, Any]] = None) -> str:
    ...
    lines.extend(_render_aa_section(aa_comparisons))
    return "\n".join(lines).rstrip() + "\n"
...
def merge_result_directories(
    ...
    aa_comparisons = build_aa_comparisons(rows, bundles)
    ...
    return {
        ...
        "schedule_status": schedule_assessment["status"],
        "aa_pair_count": aa_comparisons["pair_count"],
    }
