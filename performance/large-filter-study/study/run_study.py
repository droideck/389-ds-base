...
def run(args: argparse.Namespace, *, forced_mode: str | None = None) -> int:
    ...
    try:
        ...
        for scenario_id in selected:
            ...
            if not preflight["timing_eligible"]:
                postflight = diagnostic_flight(
                    runtime=runtime, scenario_id=scenario_id,
                    scenario=scenario, filter_text=filter_text,
                    expected=expected, expected_hash=expected_hash,
                    revision=production_revision or expected_revision,
                    diagnostic_dir=output / "diagnostics",
                    phase="postflight",
                )
                ...
                if not correctness_record["pre_post_mechanism_match"]:
                    raise StudyError(
                        f"{scenario_id}: historical pre/post mechanism "
                        "diagnostics are inconsistent"
                    )
                correctness_record["evidence_status"] = (
                    "historical-mechanism-mismatch-observed"
                )
                correctness_record["timing_eligible"] = False
                profiles.append({
                    "scenario": scenario_id,
                    "status": "skipped-historical-mechanism-mismatch",
                    "evidence_status": "not-planned-after-mechanism-gate",
                    "expected_result_code": scenario["expected_result_code"],
                    "expected_ldap_result_code": ldap_result_code_value(
                        scenario["expected_result_code"], scenario_id,
                    ),
                    "actual_client_result_code": None,
                    "actual_server_result_code": None,
                    ...
                    "profile_artifact": {
                        "evidence_status": "not-produced",
                        "path": f"profiles/{scenario_id}.perf.data",
                        "sha256": None,
                        "size_bytes": None,
                    },
                })
