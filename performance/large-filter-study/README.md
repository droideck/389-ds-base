...
The screen carries additive trust checks and reporting on top of that
coverage. `LF_EXPECTED_SOURCE=<commit>` makes the preflight assert that the
installed RPM's embedded git hash is the commit under review (mismatch aborts
before any state; unset only records `provenance.json` plus a warning).
Every bundle records an `environment.json` sidecar (kernel, cpufreq
governors, loadavg, steal-tick and thermal-throttle deltas; each source
degrades to "unavailable"). REPORT.md's off→on benefit cells carry noise
markers against a band computed from the no-effect strata (`†` exceeds the
band, `~` within it, trailing `‡` for an incoherent elapsed-only/CPU-only
move), and a generated `statistical-appendix.md` adds per-stratum
Mann-Whitney tests with effect sizes in milliseconds, the zero-candidate
CPU-versus-branch-count µs/branch slope, and within-bundle plus wall-clock
drift diagnostics. Workloads regenerated from the current spec add a
`drift-canary-<index-config>` scenario to every bundle (the wall-clock drift
anchor) and sub-threshold `branch-count-{4,8}-*` ladder rungs; older
workloads keep working with a documented fallback. `REPORT.md` also lists
per-bundle attempt hygiene (which attempt was used, what was archived to
`incomplete/` and why). All legacy report columns, fields, and raw numbers
are preserved; new CSV columns and `screen-summary.json` fields are only
appended, and `bin/regenerate-screen-report RUN_DIR --output DIR` re-renders
any existing run directory — old runs included — without modifying it.
...
When a merge contains the same binary and configuration measured as separate
sessions (for example, the custom-off state run twice into different
directories), the merger additionally reports their per-session medians and
deltas under an appended "A/A same-configuration deltas (diagnostic,
non-release)" section and `aa_comparisons` key — an empirical noise yardstick
that never becomes release evidence or a gate input. Existing pooling
behavior is unchanged, and drift-canary strata are reported as a separate
repeatability sequence instead of being paired.
