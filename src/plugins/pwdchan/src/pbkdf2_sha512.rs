...
impl SlapiPlugin3 for PwdChanPbkdf2Sha512 {
    ...
    fn start(pb: &mut PblockRef) -> Result<(), PluginError> {
        log_error!(ErrorLevel::Trace, "PBKDF2-SHA512 plugin starting");
        ...
        log_error!(
            ErrorLevel::Info,
            "PBKDF2-SHA512 plugin started successfully"
        );
        Ok(())
    }

    fn close(_pb: &mut PblockRef) -> Result<(), PluginError> {
        log_error!(ErrorLevel::Trace, "PBKDF2-SHA512 plugin closing");
        Ok(())
    }
