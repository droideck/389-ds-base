...
#define TP_STATS_WORKER_SLOT_SIZE 640
#define TP_STATS_BT_MAX_FRAMES 64
...
/*
 * Thread-pool status mmap ABI, version 1.0.
 *
 ...
 *
 * Backtrace fields are reserved for a later dsctl thread-pool-backtrace task.
 * In this MVP they are zeroed at file creation and never written again.
 * Future writer protocol:
 *   - store bt_captured = 0
 *   - fill bt_frames, bt_frame_count, and bt_timestamp_ns with relaxed stores
 *   - store bt_captured = 1 with release ordering
 * Future reader protocol:
 *   - record the trigger CLOCK_MONOTONIC timestamp
 *   - load bt_captured with acquire ordering
 *   - accept the capture only when bt_timestamp_ns is newer than the trigger
 */
typedef struct __attribute__((aligned(64))) tp_worker_slot {
    ...
    uint64_t op_id;
    uint64_t start_ns;
    uint32_t bt_captured;
    uint32_t bt_frame_count;
    uint64_t bt_timestamp_ns;
    uint64_t bt_frames[TP_STATS_BT_MAX_FRAMES];
    uint8_t reserved[80];
} tp_worker_slot_t;
