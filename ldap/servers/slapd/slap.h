 ...
 */
struct slapi_filter_or_lookup; /* slapi-private.h; per-operation OR-of-equality lookup table */
struct slapi_filter
{
    int f_flags;
    ...
    /*
     * Per-operation equality-lookup table for large OR nodes. Built only on
     * the backend's private per-search filter dups (never on shared/pblock
     * filters), owned by this node, never copied by slapi_filter_dup, freed
     * unconditionally in slapi_filter_free. NULL everywhere else.
     */
