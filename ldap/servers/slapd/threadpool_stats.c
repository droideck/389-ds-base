...
static uint64_t
tp_stats_timespec_to_ns(struct timespec ts)
{
...
{
    return tp_stats_timespec_to_ns(slapi_current_rel_time_hr());
}
...
static uint64_t
tp_load_64(uint64_t *ptr, int memorder)
{
    return slapi_atomic_load_64(ptr, memorder);
}
...
tp_stats_get_slot(uint32_t worker_idx)
{
    ...
    return (tp_worker_slot_t *)((uint8_t *)tp_stats_header +
                               TP_STATS_HEADER_SIZE +
                               ((size_t)(worker_idx - 1) * TP_STATS_WORKER_SLOT_SIZE));
}
...
tp_stats_make_path(void)
{
    ...
    if (rundir == NULL) {
        rundir = slapi_ch_strdup("/tmp");
    }

    if (configdir != NULL) {
        instname = PL_strrstr(configdir, "slapd-");
        if (instname == NULL) {
            instname = PL_strrstr(configdir, "/");
            if (instname != NULL) {
                instname++;
            }
        }
...
tp_stats_as_entry(Slapi_Entry *e)
{
    ...
    for (uint32_t i = 0; i < tp_stats_max_workers; i++) {
        char buf[256];
        char op_buf[32];
        uint32_t state;
        uint32_t op_tag;
        ...
        tp_worker_slot_t *slot = (tp_worker_slot_t *)((uint8_t *)header +
                                                      TP_STATS_HEADER_SIZE +
                                                      ((size_t)i * TP_STATS_WORKER_SLOT_SIZE));
        ...
        op_id = tp_load_64(&slot->op_id, __ATOMIC_RELAXED);
        start_ns = tp_load_64(&slot->start_ns, __ATOMIC_RELAXED);
        /* Same rule as the dsctl reader: no duration without an in-flight op */
        if (op_id != 0 && start_ns != 0 && now_ns >= start_ns) {
