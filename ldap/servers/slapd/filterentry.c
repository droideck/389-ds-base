...
/*
 * vattr_test_filter_or_lookup - evaluate a large OR through its
 * per-operation equality-lookup table (filter_or_lookup.c).
 *
 * Contract: identical three-valued semantics to
 * vattr_test_filter_list_or - 0 iff some component is access-allowed
 * AND matches; else -1 if any component is defined-false; else the
 * undefined (>0) class.  The table only selects WHICH components to
    ...
    int32_t *decided)
{
    ...
    *decided = 0;
    ...
    /* A virtually-suppliable type must use the classic walk, which
     * consults the service providers per component. */
    if (vattr_type_sp_registered(e, ol->ol_type)) {
        return 0;
    }

    /* DN-syntax entries with more values than the table has keys are
     * cheaper on the classic walk: the sorted-valueset find there is
     * O(log m) per component, while probing costs one DN normalization
     * per value.  (String-family components re-normalize the entry
     * values on every component today, so no such guard applies.) */
    if (ol->ol_type_is_dn) {
        size_t m = 0;
        for (a = e->e_attrs; a != NULL; a = a->a_next) {
    ...
    for (a = e->e_attrs; a != NULL; a = a->a_next) {
        ...
        /* Same subtype predicate as the per-component scan in
         * test_ava_filter: a base-type component matches values of
         * subtyped instances too. */
        if (slapi_attr_type_cmp(ol->ol_type, a->a_type, SLAPI_TYPE_CMP_SUBTYPE) != 0) {
            continue;
        }
        ...
        for (i = 0; va != NULL && va[i] != NULL; i++) {
            ...
            /* The normalizers modify their input in place - never hand
             * them the cached entry's own value buffer. */
            ...
            if (ol->ol_type_is_dn) {
                ...
                if (nrc < 0) {
                    /* Unparseable stored value: today's walk compares it
                     * raw-vs-raw, which the table cannot reproduce. */
                    norm_failed = 1;
                } else if (nrc == 0) {
                    probe.bv_val = dest; /* in place, not NUL terminated */
                    probe.bv_len = dlen;
                } else {
                    norm = dest;
                    probe.bv_val = dest;
                ...
                /* Identical normalizer + filter type to what
                 * filter_normalize_ava applied to the table keys. */
            ...
            probe_hits++;
            if (verify_access) {
                /* Access first, on exactly the component the value hit -
                 * same (type, value) pair the linear walk would check. */
                rc = slapi_vattr_filter_test_ext_internal(pb, e, branch, verify_access,
                                                          -1, access_check_done);
                if (rc != 0) {
                    /* No access to this component: ignore it and keep
                     * probing - another value may hit an allowed one. */
                    continue;
                }
            }
            ...
            /* rc > 0, or the confirm disagreed with the probe: keep
             * scanning; the no-winner logic below decides. */
        }
    }
